---
{"dg-publish":true,"dg-path":"A1- 数学/2. 级数/3.3-复变函数项级数.md","permalink":"/A1- 数学/2. 级数/3.3-复变函数项级数/","dgPassFrontmatter":true,"noteIcon":"","created":"2024-10-06T20:06:48.143+08:00","updated":"2025-09-06T17:36:59.167+08:00"}
---
...
注意区分**系数**和展开的**幂**，**收敛半径**是通过**系数**定义的
- > $c_{n}$ 为与 $n$ 有关的系数
...
### 四、在收敛圆内的性质
幂级数 $\sum\limits_{n=0}^{\infty}c_{n}(z-z_{0})^{n}$ 的收敛半径为 $R$，收敛圆 $|z-a|<R$ 
...
$$\int _{a}^{z} f(\zeta)\, d\zeta=\sum\limits_{n=0}^{\infty} \dfrac{c_{n}}{n+1}(z-a)^{n+1}$$
>[!important] 注意
...
所以原级数：
$$\begin{align}
\sum\limits_{n=1}^{\infty}(2^{n}-1)z^{n-1}&=\dfrac{2}{1-2z}- \dfrac{1}{1-z} =\dfrac{1}{(1-2z)(1-z)}
\end{align}$$
