...
- **前端 (Frontend)**: 也称为“视觉里程计 (Visual Odometry)”或“激光里程计”。它负责处理实时的传感器数据，进行特征提取、数据关联，并估算短时间内的、局部的机器人运动。
- **后端 (Backend)**: 负责接收前端的信息，并进行全局的状态估计和优化。后端是实现高精度和全局一致性的关键，如图优化就是在后端完成的。
...
> 用递归滤波（如扩展卡尔曼滤波 EKF 或粒子滤波）维护机器人状态和地图状态的联合概率分布，**递推地**估计机器人**当前时刻**的状态
...
- **工作方式**: 只维护当前时刻的状态估计，当新的数据到来时，用它来更新当前状态。它不保留所有历史信息。
- **特点**: 计算效率高，内存占用小。但由于线性化近似和信息丢弃，其精度和长期一致性会受到影响。
- **优点：** 理论成熟，实时性好。
...
- **工作方式**: 将 SLAM 问题构建为一个图。图中的**节点 (Nodes)** 代表机器人经过的每一个历史位姿和环境中的路标点。图中的**边 (Edges)** 代表由运动模型或观测模型产生的约束（即误差）。SLAM 的目标就变成了**调整图中所有节点的位置**，使得整个图的**总误差最小**。
- **特点**: 能够利用所有历史信息进行全局优化，因此**精度更高、更鲁棒**。但计算量和存储需求也更大。当检测到**回环 (Loop Closure)**（即机器人回到了一个曾经到过的地方）时，图优化方法可以非常有效地消除累积误差。
